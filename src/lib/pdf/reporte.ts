...
export async function descargarPDF(
	...
	const filasBody_page1 = equipo
		...
		.map((puesto) => {
			...
			const fila: (string | number)[] = [`${puesto.numero}. ${datosJugador.apellido}, ${datosJugador.nombre}`];
	...
	autoTable(doc, {
		...
		didDrawPage: (data: HookData) => {
			...
			const textoAclaracion1 = doc.splitTextToSize(
				'Solo los tackles negativos, positivos y dominantes cuentan como efectivos.',
				anchoMaxCol3
			);
			const textoAclaracion2 = doc.splitTextToSize(
				'Solo los duelos positivos y quiebres cuentan como efectivos.',
				anchoMaxCol3
			);
	...
	const nombreArchivo = `Reporte_${partido.local}_vs_${partido.visitante}.pdf`.replace(/\s+/g, '_');
