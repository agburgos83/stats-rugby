...
export type Contador = {
	Negativo: number; Neutro: number; Positivo: number; Dominante: number; Total: number;
};
export type FilaJugador = {
	id: number; nombre: string; apellido: string;
	skills: Record<Skill, Contador>;
	totalGeneral: number; totalPelota: number; totalContacto: number; totalPie: number;
	efectividadPelota: number; efectividadContacto: number; efectividadPie: number;
};
export type MatrizProcesada = Record<number, FilaJugador>;
export type DixTotales = Record<string, [number, number]>;

function crearContadorVacio(): Contador {
...
export function procesarReporte(
	...
	for (const puesto of equipo) {
		...
		const dixSkills: Record<string, Contador> = {};
